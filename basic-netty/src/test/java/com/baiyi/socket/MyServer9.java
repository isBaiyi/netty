		...
		while (true) {
			...
			while (iterator.hasNext()) {
				...
				if (key.isAcceptable()) {
					...
					System.out.println("第一次写的数据对应的write = " + write);
					// 第一次发完之后如果还没写完，就监听一下可写状态
					if (buffer.hasRemaining()) {
						scKey.interestOps(scKey.interestOps() + SelectionKey.OP_WRITE);
						// 把buffer和sc绑定起来，以附件的形式传递下去，这样这个buffer就 能通过sc获取了，可以跨越if分支
						scKey.attach(buffer);
					}
					...
					if (buffer.hasRemaining()) {
						...
						System.out.println("监听可写事件写的数据对应的write = " + write);
						key.attach(buffer);
					} else {
						// 取消监听写操作
